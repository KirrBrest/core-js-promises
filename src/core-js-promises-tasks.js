...
function getPromise(number) {
  const promise = new Promise(function f(resolve, reject) {
    if (number >= 0) {
      resolve('done');
    } else {
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((resource) =>
    resource.map((el) => (el.status === 'fulfilled' ? el.value : null))
  );
}
...
function queuPromises(promises) {
  return promises.reduce((acc, promise) => {
    return acc.then((res) => {
      return promise.then((el) => res + el.toString());
    });
  }, Promise.resolve(''));
}
